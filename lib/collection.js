...
function Collection(component) {
  ...
  var handler$Hash = arguments.length <= 4 || arguments[4] === undefined ? {} : arguments[4];
  var reducers = arguments.length <= 5 || arguments[5] === undefined ? _xstream2.default.create() : arguments[5];
  ...
  return {
    add: function add() {
      ...
      return Collection(component, sources, handlers, [].concat(_toConsumableArray(items), [newItem]), _extends({}, handler$Hash, _defineProperty({}, newItem.id, handler$)), reducers);
    },
    remove: function remove(itemForRemoval) {
      ...
      return Collection(component, sources, handlers, items.filter(function (item) {
        return item !== itemForRemoval;
      }), _extends({}, handler$Hash, _defineProperty({}, id, null)), reducers);
    },
